...
`rd-workflow/extensions/verify/verifiers/xcuitest-macos.sh`를 아래 내용으로 채웁니다:
...
```bash
cp rd-workflow/extensions/verify/presets/macos/verification.json rd-workflow/config/verification.json
```
...
```bash
bash rd-workflow/extensions/verify/verify.sh --all
```

특정 verifier만 실행:

```bash
bash rd-workflow/extensions/verify/verify.sh --verifier xcuitest-macos
```
...
|----------------|-------------------------------------------|------------------|
| xcuitest-macos | `rd-workflow/extensions/verify/verifiers/xcuitest-macos.sh` | Xcode CLI Tools  |
...
`rd-workflow/config/verification.json`을 열고 각 criteria의 `description`에 표시된 `TODO:` 항목을 프로젝트 실정에 맞게 채웁니다.
...
`rd-workflow/extensions/verify/verifiers/xcuitest-macos.sh`의 `run` 명령어를 프로젝트 설정에 맞게 수정할 수 있습니다:
...
- `rd-workflow/extensions/verify/verifiers/xcuitest-macos.sh`를 프로젝트의 XCUITest 구조에 맞게 작성해야 합니다.
