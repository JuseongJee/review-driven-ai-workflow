...
7. **loop-guard 임계 도달** (`loop_guard_check` 가 return 1): 동일 검증 연속 실패·동일 파일 churn+검증실패·반복 rollback 이 임계를 넘으면 즉시 `awaiting-user` 로 전환하고 사유(loop_guard_check stdout)를 보고한다. 이는 review 턴 한도·디버깅 3회와 독립된 별도 중단 조건이다.
