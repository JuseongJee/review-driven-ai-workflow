...
- `CURRENT_TASK.md`
- `ai/config/verification.json` (없으면 skip)
- `CURRENT_TASK.md`의 Spec 경로에 해당하는 spec의 `## Verification Criteria`

## 진입 조건

`ai/config/verification.json`이 없으면:
...
`ai/config/verification.json`에서 verifier 목록을 읽고 사용자에게 제시한다:
...
```bash
bash ai/extensions/verify/verify.sh --all                    # 전체 실행
bash ai/extensions/verify/verify.sh --skip playwright,axe     # 제외 항목 있으면
```
...
1. 코드 수정
2. `bash ai/extensions/verify/verify.sh --all` 재실행
3. 새 결과로 Author 턴 작성
4. Reviewer 재평가
