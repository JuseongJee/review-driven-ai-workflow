...
이 파일은 verify extension 설치 시 `ai/claude_skills/verify/`에 복사되어 자동으로 활성화된다.

## 적용 시점
- 구현·검증(test/lint/typecheck) 완료 후, final-diff-review 전
- `ai/config/verification.json`이 존재할 때

## 동작
- AI가 "verification.json이 있네요. 런타임 검증을 돌릴까요?"로 추천
- 사용자가 수락하면 /verify skill 실행
- 사용자가 거절하면 기본 흐름 그대로 진행
- 검증 실행: `bash ai/extensions/verify/verify.sh` (extension 디렉토리에서 직접 실행)
