...
메모:
  ...
  - `project`는 `ai/claude_skills/`를 `.claude/skills/`로 bootstrap합니다.
  - `personal`은 `ai/claude_skills/`를 `~/.claude/skills/`로 설치합니다.
EOF
}
...
skills_source_dir="${source_root}/ai/claude_skills"
if [[ ! -d "$skills_source_dir" ]]; then
  echo "Claude skill source directory not found: $skills_source_dir" >&2
  echo "This script expects skill source files to already exist at ai/claude_skills." >&2
  echo "Creating an empty directory is not enough; the skill folders and SKILL.md files must be there." >&2
  echo "In a copied project, this usually means ai/claude_skills was not copied." >&2
  echo "Copy ai/claude_skills from the template as well, then rerun this installer." >&2
  exit 1
fi
