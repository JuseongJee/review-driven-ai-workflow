...
AI가 `ai/docs/guides/setup_with_claude.md` 절차를 따릅니다:

1. 템플릿 파일 복사 (배포 repo → 프로젝트 루트)
2. `PROJECT_CONTEXT.md` 채우기 (프로젝트 타입, 기술 스택, 빌드 명령 등)
3. 검증 스크립트 설정 (`ai/scripts/test.sh`, `lint.sh`, `typecheck.sh`)
4. 스킬 설치 (`ai/scripts/install_claude_skills.sh`)
5. 리뷰 도구 감지 (Codex/Gemini 설치 여부 확인 + 안내)
6. (선택) 확장 기능 설치, PROJECT_CONTEXT review

### 2.2 필수 설정 파일

설치 후 `ai/config/` 아래 example 파일을 복사하여 설정합니다:

```bash
cp ai/config/workflow.json.example ai/config/workflow.json
...
`ai/docs/guides/migrate_existing_project.md` 절차를 따릅니다.

### 2.4 팀 프로젝트에 개인 설치

팀 repo를 오염시키지 않고 개인 private repo로 분리 설치:

- **처음 설치**: [team-overlay-fresh-install.md](ai/docs/guides/team-overlay-fresh-install.md)
- **이미 설치된 프로젝트에서 분리**: [team-overlay-migration.md](ai/docs/guides/team-overlay-migration.md)
...
| 도구 | 역할 | 설정 |
|------|------|------|
| **Codex (기본)** | 독립 Reviewer | `ai/scripts/adapter_codex.sh` |
| **Gemini** | 독립 Reviewer | `ai/scripts/adapter_gemini.sh` |
| **Claude** | Self-review fallback | `ai/scripts/adapter_claude.sh` |

우선순위는 `ai/config/review-tools.json`의 `default_priority`로 설정.

---

## 7. 설정 파일

모든 설정은 `ai/config/`에 위치합니다. `.example` 파일을 복사하여 사용합니다.
...
- `ai/config/verification.json`에서 도구 설정 (Lighthouse, Playwright, axe 등)
...
설치 상태는 `ai/config/extensions.json`에 기록됩니다.
...
| 스크립트 | 용도 | 예시 |
|---------|------|------|
| `ai/scripts/build.sh` | 빌드 | `npm run build` |
| `ai/scripts/test.sh` | 테스트 | `npm test` |
| `ai/scripts/lint.sh` | 린트 | `npm run lint` |
| `ai/scripts/typecheck.sh` | 타입 체크 | `tsc --noEmit` |
...
| 스크립트 | 용도 |
|---------|------|
| `ai/scripts/prepare_review_pipeline.sh <종류>` | 리뷰 세션 생성 (`request` / `spec-plan` / `diff`) |
| `ai/scripts/run_review_turn.sh <session-path>` | 리뷰 턴 한 회 실행 |
| `ai/scripts/review_common.sh` | 공통 함수 (직접 실행 안 함) |

### 9.3 기타 스크립트

| 스크립트 | 용도 |
|---------|------|
| `ai/scripts/install_claude_skills.sh` | 스킬을 Claude Code에 설치 (link/copy) |
| `ai/scripts/sync_template.sh` | 배포 repo에서 최신 템플릿 동기화 |
| `ai/scripts/check_claudemd_size.sh` | CLAUDE.md 200줄 초과 경고 |
| `ai/scripts/adapter_codex.sh` | Codex 리뷰 어댑터 |
| `ai/scripts/adapter_gemini.sh` | Gemini 리뷰 어댑터 |
| `ai/scripts/adapter_claude.sh` | Claude 리뷰 어댑터 (self-review) |
...
→ `ai/config/review-tools.json`에서 우선순위를 변경하거나, `/review-config setup`으로 재설정하세요. Claude self-review가 fallback입니다.
...
| 상황 | 프롬프트 파일 |
|------|-------------|
| REQUEST부터 plan까지 한 번에 | `ai/docs/prompts/recovery/request_to_reviewed_plan_full.md` |
| Superpowers 없이 plan 작성 | `ai/docs/prompts/recovery/write_plan_without_skill.md` |
| 리뷰 수동 시작 | `ai/docs/prompts/manual/review_pipeline_start_manual.md` |
| 리뷰 수동 이어가기 | `ai/docs/prompts/manual/review_pipeline_continue_manual.md` |
