...
- 템플릿 관련 파일(`ai/`, `CLAUDE.md`, `.claude/`)을 팀 repo에 커밋하고 싶지 않다
...
cp -r /path/to/ai-dev-template/ai/ .
...
# 평소 작업 — ai/ 파일은 symlink를 통해 자연스럽게 접근
# Claude Code, autopilot 등 모두 정상 동작

# 팀 작업 커밋 (ai/ 파일은 .gitignore/exclude로 무시됨)
git add src/
...
- `ai/` 내부에서 절대경로를 사용하는 스크립트가 있으면 symlink 환경에서 깨질 수 있다. 상대경로를 사용한다.
