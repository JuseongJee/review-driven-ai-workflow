33333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333# Team Overlay Migration 가이드 (기존 프로젝트)

이미 팀 repo에 AI 워크플로 템플릿 파일이 커밋되어 있는 프로젝트에서, 템플릿 파일을 개인 private repo로 분리하는 방법.

## 언제 사용하는가

- 팀 프로젝트에 이미 `ai/`, `CLAUDE.md` 등이 커밋되어 있다
...
- 팀 repo에 대한 커밋 권한33333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333이 있는지 확인한다 (방법 선택에 영향)
...
rm -rf ai/ CLAUDE.md CURRENT_TASK.md REQUEST.md PROJECT_CONTEXT.md .claude/
...
- `git pull` 시 ai/ 파일이 삭제됨 → 팀원이 이 파일을 안 쓰고 있었다면 영향 없음
...
git ls-files ai/ CLAUDE.md CURRENT_TASK.md REQUEST.md PROJECT_CONTEXT.md .claude/ 2>/dev/null | \
...
- `git pull`로 팀이 ai/ 파일을 수정하면: 충돌 가능 → skip-worktree 재설정 필요
...
- [ ] `ls -la ai/` → symlink 확인 (`ai -> ~/ai-overlays/...`)
- [ ] `git status` → ai/, CLAUDE.md 등이 untracked/ignored로 표시
- [ ] `cat ai/docs/flows/WORKFLOW.md` → 정상 읽힘
