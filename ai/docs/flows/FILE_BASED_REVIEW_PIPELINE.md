# File-based Codex-Claude Review Pipeline
...
- Codex 차례가 되면 Claude가 adapter를 실행해 Codex 턴 파일을 만듭니다
...
- `turns/NNN_<agent>.md`: Claude / Codex 턴 기록
...
2. Claude가 세션 파일과 검토 대상을 읽습니다
3. Claude가 자기 턴 파일 하나를 씁니다
4. Codex 차례가 되면 Claude가 `run_review_turn.sh codex ...`를 실행합니다
5. 최신 Codex 턴에 `이의 없음`이 나올 때까지 3~4 단계를 반복합니다
...
- `ai/scripts/ai/run_review_turn_codex.sh`
